...
import { PlayerConditionDetail, playerConditionEmpty } from "../types/PlayerConditionDetail";
...
const OrPlayerConditionList: React.FC<Props> = ({ detail, onChanged }) => {
  ...
  const onOrPlayerConditionsChanged = (x: PlayerConditionDetail, index: number) => {
    const newlist = [...detail.orPlayerConditions];
    ...
    >
      {detail.orPlayerConditions.map((e, index) => (
        <FormGroup key={index}>
          ...
          <PlayerCondition detail={e} onChanged={(x) => onOrPlayerConditionsChanged({ ...e, ...x }, index)}></PlayerCondition>
        </FormGroup>
      ))}
    </FormSet>
  );
